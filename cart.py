...
Arbol de decisión tipo Cart

"""

__author__ = 'nethojs29'
...
class NodoCART(object):
    """
    Nodo para un árbol de desición tipo Cart.

    """
    def __init__(self, clase, atributo=None,valor=None):
        self.clase = clase
    ...
    def asigna(self, x):
        """
        Asignación individual tipo Cart, 
...
class ArbolDecision(object):
    ...
    def construye_arbol(self, datos, clases, atributos_utilizados, clase_default, prepoda=(0, 1.0, 0.0001)):
        ...
        nodo.hijos["Y"] = self.construye_arbol([x for x in datos if x[atributo] == valor],
        										[t for (t,x) in zip(clases,datos) if x[atributo] == valor], 
        										atributos_utilizados, clase_actual)
        nodo.hijos["N"] = self.construye_arbol([x for x in datos if x[atributo] != valor], 
        										[t for (t,x) in zip(clases,datos) if x[atributo] != valor], 
        										atributos_utilizados,clase_actual)
